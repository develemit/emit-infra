...
- (sprint 234, 2026-07-24) tastease's nginx drift is cosmetic (upstream variable renames: `api_upstream`/`web_upstream`/`marketing_upstream` on the server vs. `tastease_api`/`tastease_web`/`tastease_marketing` in the repo) rather than a routing-safety issue — the `/api/` location split survives on both sides and is live-confirmed working. Still worth syncing to remove the divergence; verify the server's blue-green include defines the old upstream names before assuming self-consistency.
