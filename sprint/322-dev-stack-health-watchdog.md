...
## Acceptance criteria
- [ ] With the API stopped and the job still loaded, the watchdog kickstarts `com.emit.infra` after the threshold and the API answers `/health` again
- [ ] After `pnpm launch:stop`, the watchdog takes no action on any number of runs
- [ ] A healthy stack produces no kickstart and no log spam
- [ ] The hourly ceiling prevents more than N kickstarts and logs a give-up line
- [ ] `scripts/lib/dev-stack-watchdog.test.sh` covers all five cases in task 7 and is registered in `pnpm test:hooks`
