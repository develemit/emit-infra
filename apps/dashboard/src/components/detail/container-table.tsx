...
function MContainer({ c }: { c: Container }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden" style={{ padding: 18 }}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <Badge variant="muted" mono className="ml-1">{runningCount} running</Badge>
      </div>
      ...
      {containers.length === 0 ? (
        ...
        <>
              ...
              <tbody>
                {containers.map(c => (
                  ...
                  >
                    ...
                    <td className="font-mono text-[12px] text-subtle py-3">{c.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="lg:hidden flex flex-col gap-2">
            {containers.map(c => <MContainer key={c.name} c={c} />)}
          </div>
        </>
      )}
    </div>
